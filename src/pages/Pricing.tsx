...
import PlanExpiryBadge from '@/components/dashboard/PlanExpiryBadge';
...
export default function Pricing() {
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      ...
      try {
        ...
        // Safe access since we selected the fields
        const subscription = data as { plan: string; plan_expires_at: string | null };
        setCurrentPlan(subscription?.plan || 'free');
        setPlanExpiresAt(subscription?.plan_expires_at || null);
      } catch (err) {
  ...
  const handleDowngrade = async () => {
    ...
    setDowngrading(true);
    try {
      const { error } = await supabase.functions.invoke('downgrade-to-free');
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-6xl mx-auto">
            ...
            {currentPlan !== 'free' && planExpiresAt && (
              <div className="col-span-full flex items-center justify-center gap-3 p-3 bg-primary/10 rounded-lg border border-primary/20">
                <span className="text-sm text-foreground">Your current plan:</span>
                <Badge className="bg-primary/20 text-primary border-primary/30 uppercase">{currentPlan}</Badge>
                <PlanExpiryBadge plan={currentPlan} expiresAt={planExpiresAt} />
              </div>
            )}
