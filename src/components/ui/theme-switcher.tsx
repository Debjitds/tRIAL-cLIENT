...
export const ThemeSwitcher = () => {
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      const currentTheme = (localStorage.getItem("theme") as TTheme) || "system";
      setThemeState(currentTheme);
    };

    window.addEventListener("theme-change", handleStorageChange);
    
    const handleGlobalStorage = (e: StorageEvent) => {
      if (e.key === "theme") handleStorageChange();
    };
  ...
  return (
    <fieldset className="flex h-8 p-0 m-0 rounded-[999999px] border border-border bg-surface">
      <legend className="sr-only">Select a display theme:</legend>
      <div className="mt-[-1px] ml-[-1px]">
        <input
          aria-label="system"
          id={`theme-switch-system-${Math.random().toString(36).substr(2, 9)}`}
          type="radio"
          value="system"
        ...
        />
        <label
          htmlFor={`theme-switch-system-${Math.random().toString(36).substr(2, 9)}`}
          className={clsx(
            "flex items-center justify-center h-8 w-8 cursor-pointer rounded-[999999px] group transition-all duration-200",
            theme === "system" && "bg-gradient-primary hover-glow shadow-sm"
          )}
        ...
        >
          ...
          <svg height="16" strokeLinejoin="round" viewBox="0 0 16 16" width="16">
            <path
              ...
              className={clsx(
                "group-hover:fill-foreground duration-150 transition-colors",
                theme === "system" ? "fill-white" : "fill-foreground-secondary"
              )}
            />
          </svg>
        </label>
      ...
      <div className="mt-[-1px]">
        <input
          aria-label="light"
          id={`theme-switch-light-${Math.random().toString(36).substr(2, 9)}`}
          type="radio"
          value="light"
          checked={theme === "light"}
        ...
        />
        <label
          htmlFor={`theme-switch-light-${Math.random().toString(36).substr(2, 9)}`}
          className={clsx(
            "flex items-center justify-center h-8 w-8 cursor-pointer rounded-[999999px] group transition-all duration-200",
            theme === "light" && "bg-gradient-primary hover-glow shadow-sm"
          )}
        ...
        >
          ...
          <svg height="16" strokeLinejoin="round" viewBox="0 0 16 16" width="16">
            <path
              ...
              className={clsx(
                "group-hover:fill-foreground duration-150 transition-colors",
                theme === "light" ? "fill-white" : "fill-foreground-secondary"
              )}
            />
          </svg>
        </label>
      ...
      <div className="mt-[-1px] mr-[-1px]">
        <input
          aria-label="dark"
          id={`theme-switch-dark-${Math.random().toString(36).substr(2, 9)}`}
          type="radio"
          value="dark"
          checked={theme === "dark"}
        ...
        />
        <label
          htmlFor={`theme-switch-dark-${Math.random().toString(36).substr(2, 9)}`}
          className={clsx(
            "flex items-center justify-center h-8 w-8 cursor-pointer rounded-[999999px] group transition-all duration-200",
            theme === "dark" && "bg-gradient-primary hover-glow shadow-sm"
          )}
        ...
        >
          ...
          <svg height="16" strokeLinejoin="round" viewBox="0 0 16 16" width="16">
            <path
              ...
              className={clsx(
                "group-hover:fill-foreground duration-150 transition-colors",
                theme === "dark" ? "fill-white" : "fill-foreground-secondary"
              )}
            />
          </svg>
        </label>
