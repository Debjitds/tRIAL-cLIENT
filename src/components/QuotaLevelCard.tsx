      ...
      >
        <CardHeader className="text-center pb-6 pt-8">
          ...
          <div className="mt-4 space-y-2 text-sm">
            ...
            {/* Credits available - ALWAYS show for veteran (no free quota) or when free quota exhausted */}
            {remaining !== 'locked' && typeof remaining === 'number' && (remaining === 0 || level === 'veteran') && (
              <div className={`flex justify-between items-center px-4 py-2 rounded-lg border ${
                ...
                <span className={`font-semibold ${availableCredits >= creditCost ? 'text-accent' : 'text-destructive'}`}>
                  {availableCredits}
                </span>
              </div>
            )}
            
            {/* Insufficient credits warning with Get Credits button - show for veteran always or when quota exhausted */}
            {remaining !== 'locked' && typeof remaining === 'number' && (remaining === 0 || level === 'veteran') && availableCredits < creditCost && (
              <div className="flex flex-col gap-2 px-3 py-2 bg-warning/10 rounded-lg border border-warning/30">
                ...
                <Button
                  ...
                  size="sm"
                  className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-primary hover:bg-primary/90 text-primary-foreground text-xs font-medium"
                >
